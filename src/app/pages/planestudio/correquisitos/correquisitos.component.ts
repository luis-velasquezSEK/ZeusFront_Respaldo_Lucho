...
@Component({
  selector: 'app-correquisitos',
  templateUrl: './correquisitos.component.html',
  styleUrls: ['./correquisitos.component.scss']
})
export class CorrequisitosComponent implements OnInit {
  ...
  agregarArchivos() {
    ...
    console.log('TAMAÑO CORREQUISITOS: ' + this.fileInput.files.length);
    //this.startUpload();

    if (this.fileInput.files.length > 0) {
      for(let obj of this.fileInput.files) {
  ...
  submit() {
    this.fileService.uploadSingleFile(this.form.get('avatar').value, "empleado", "avatar-.png")
      .subscribe(event => {
  ...
  imageURL1: string = "http://i.pravatar.cc/500?img=7";
  ...
  addArchivos() {
    ...
    let files:File[]=[];
    files.push(this.form.get('avatar').value);
    files.push(this.form.get('avatar1').value);

    //add files to uploadedFiles and send to server async await
    this.fileService.uploadManyFiles(files, "empleado")
      .subscribe(event => {
        console.log(event.type);
      });
